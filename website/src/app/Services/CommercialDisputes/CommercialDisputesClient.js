...
// Add to ANY existing page in website/src/app/
const API = process.env.NEXT_PUBLIC_SERVER_URL || 'http://localhost:5000';

export async function generateMetadata() {
  try {
    const res  = await fetch(`${API}/api/cms/pages/slug/commercialdisputes`, { cache: 'no-store' });
    const data = await res.json();
    const seo  = data?.seo || {};
    return {
      title:       seo.title       || 'Commercial Disputes | RaaziMarzi',
      description: seo.description || 'Resolve commercial disputes online.',
      openGraph: {
    ...
    return { title: 'Commercial Disputes | RaaziMarzi' };
  }
}
/* ── Data ── */
const disputeTypes = [
...
export default function CommercialDisputesClient() {
