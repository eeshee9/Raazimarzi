// components/seo/tabs/SocialTab.jsx
'use client';
...
function Field({ label, hint, children }) {
  return (
    <div style={{ marginBottom: 14 }}>
      ...
      {children}
      {hint && (
        <p style={{ fontSize: 11, color: 'var(--color-text-tertiary)', marginTop: 3, lineHeight: 1.4 }}>{hint}</p>
      )}
    </div>
  );
}
...
function OgCard({ title, description, image, url }) {
  return (
    <div style={{ border: '0.5px solid #dddfe2', overflow: 'hidden', marginBottom: 4, fontFamily: 'Helvetica, Arial, sans-serif', background: '#f0f2f5' }}>
      ...
      <div style={{ padding: '8px 12px', background: '#f0f2f5', borderTop: '0.5px solid #dddfe2' }}>
...
function TwitterCard({ title, description, image, card }) {
  ...
  return (
    <div style={{ border: '0.5px solid #e1e8ed', borderRadius: 14, overflow: 'hidden', fontFamily: '-apple-system, BlinkMacSystemFont, sans-serif', background: '#fff' }}>
      {isLarge && (
    ...
    <div>
      ...
      <div style={{ marginBottom: 24 }}>
        ...
        <OgCard
          ...
          description={ogDesc}
          image={seo.og.image}
          url={seo.slug ? `${SITE_URL}/${seo.slug}` : SITE_URL}
        />
        <p style={{ fontSize: 11, color: 'var(--color-text-tertiary)', marginBottom: 10 }}>
          Falls back to main title/description if left blank.
        </p>

        <Field label="OG title override" hint="Max 60 chars recommended for Facebook.">
          <input type="text" value={seo.og.title} onChange={e => onChange({ og: { ...seo.og, title: e.target.value } })} placeholder={seo.title || 'Leave blank to use SEO title'} style={inputStyle} maxLength={80} />
        </Field>
        <Field label="OG description override">
          <textarea value={seo.og.description} onChange={e => onChange({ og: { ...seo.og, description: e.target.value } })} placeholder={seo.description || 'Leave blank to use meta description'} style={{ ...inputStyle, resize: 'vertical', minHeight: 64 }} maxLength={200} />
        </Field>
        <Field label="OG image URL" hint="Recommended: 1200×630px. Used on Facebook, LinkedIn, WhatsApp.">
          <input type="url" value={seo.og.image} onChange={e => onChange({ og: { ...seo.og, image: e.target.value } })} placeholder="https://raazimarzi.com/og/page-name.jpg" style={inputStyle} />
        </Field>
      </div>
      ...
      <div>
        ...
        <TwitterCard
          title={twTitle}
          description={twDesc}
          image={seo.twitter.image || seo.og.image}
          card={seo.twitter.card}
        />
        <p style={{ fontSize: 11, color: 'var(--color-text-tertiary)', margin: '6px 0 12px' }}>
          Falls back to OG image if Twitter image not set.
        </p>

        <Field label="Card type">
          <select value={seo.twitter.card} onChange={e => onChange({ twitter: { ...seo.twitter, card: e.target.value } })} style={inputStyle}>
        ...
        <Field label="Twitter title override">
          <input type="text" value={seo.twitter.title} onChange={e => onChange({ twitter: { ...seo.twitter, title: e.target.value } })} placeholder={seo.title || 'Leave blank to use SEO title'} style={inputStyle} maxLength={70} />
        </Field>
        <Field label="Twitter image URL" hint="Recommended: 1200×628px. Falls back to OG image.">
          <input type="url" value={seo.twitter.image} onChange={e => onChange({ twitter: { ...seo.twitter, image: e.target.value } })} placeholder={seo.og.image || 'https://raazimarzi.com/twitter/page-name.jpg'} style={inputStyle} />
        </Field>
      </div>
    </div>
  );
